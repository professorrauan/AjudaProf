...
function iniciarEscutaHistoricoFirebase() {
  ...
  /*
   * Não chamar encerrarEscutaHistoricoFirebase() aqui.
   * Essa função também cancela os eventos da tela (busca e filtro).
   * Ao iniciar a escuta em tempo real logo após montar o Histórico,
   * os listeners recém-criados eram abortados imediatamente.
   *
   * Aqui encerramos somente a escuta antiga do Firestore.
   */
  if (typeof cancelarEscutaHistoricoFirebase === "function") {
    try {
      cancelarEscutaHistoricoFirebase();
      ...
      console.warn(
        "Não foi possível encerrar a escuta anterior do Histórico:",
        erro
      );
    }
  }
...
function registrosFiltradosHistorico() {
  return historicoAtualAjudaProf.filter((registro) => {
    const termoBusca = buscaHistoricoAtual;
    ...
    const combinaNome =
      !termoBusca || textoBusca.includes(termoBusca);

    const turma = registro.turma || "Sem turma";

    const combinaTurma =
      !filtroTurmaHistoricoAtual ||
      normalizarHistorico(turma) ===
        normalizarHistorico(filtroTurmaHistoricoAtual);
